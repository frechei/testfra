 ...
 */
package org.haotao.common.database;
...
import org.haotao.common.utils.ActivityStack;
...
public class HTDB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, HTDB> daoMap = new HashMap<String, HTDB>();

    private SQLiteDatabase db;
    private final DaoConfig config;

    private HTDB(DaoConfig config) {
        if (config == null) {
    ...
    private synchronized static HTDB getInstance(DaoConfig daoConfig) {
        HTDB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new HTDB(daoConfig);
     ...
     * @param context
     */
    public static HTDB create(Context context) {
     ...
     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static HTDB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static HTDB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static HTDB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static HTDB create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static HTDB create(DaoConfig daoConfig) {
